...
function check_question_type(){
    if(questionaires[0].answer=='true' || questionaires[0].answer=='false'){
        display_data_tor()
    }
    else{
        display_data()
...
function finalize(){
    $.ajax({
        url:'question.php',
        method:'post',
        data:{"finalize":'',"user_answers":user_answers},
        dataType:'json',
        success:function(data){
            user_answers=[]
            user_answer={}
...
function promo(data){
    ...
    output = `<div class='promo'><h1 class='lead'>You're Score is ${score}=${average}% <br>${com}</h1><br>
    <a href="http://localhost/activity/" class="btn btn-success space btn-lg" role="button">Home Page</a> 
    <button type="button" class="space btn btn-info btn-lg">View Quiz Result</button></div>`;
